...
format long g
rng('shuffle')
constant.movement_mode = 0;
if constant.movement_mode == 0
	movement_data = flux_capacitor;
	clear flux_capacitor
end
% 0 for feed in
% 1 for random
...
% NOT RANDOM ENOUGH
%	x_coord = func_gen_random_int(constant.workspace_x);
%	y_coord = func_gen_random_int(constant.workspace_y);

%	x_coord = randi(100);
%	y_coord = randi(100);

	x_coord = initial_xy(counter,2);
	y_coord = initial_xy(counter,3);
...
% ====================================================================================================
constant.malicious.toggle = 1; % 0 for off, 1 for on

if constant.malicious.toggle == 1
	constant.malicious.node = 100;

	constant.good.key = 100100100;
	nodes(constant.malicious.node).key = [0,constant.malicious.node,constant.good.key];
	nodes(constant.malicious.node).key_direct = nodes(constant.malicious.node).key;
	...
	data.good.all = zeros(constant.node_no,4);
	data.good.all(:,1) = 1:constant.node_no;
	data.good.time = [];
	data.good.other_bad_direct = [];
	data.good.other_bad_direct_length = 0;
	data.good.other_bad_approved = [];
	data.good.other_bad_approved_length = 0;
	data.good.other_other_approved = [];
	data.good.other_other_approved_length = 0;
	data.good.direct_total = 0;
	data.good.approved_total = 0;
	data.good.grand_total = 0;
